...
<body>
	...
	<div class="container">
		...
		<div class="input-group mb-3">
			<input type="text" name="search" value="<?php if(isset($_GET['search'])) { echo $_GET['search']; }  ?>" class="form-control" placeholder="search the data">
		...
		<table class=" table table-bordered">
			...
			<tbody>
				...
				if (isset($_GET['search'])) 
				{
					...
					$query ="SELECT * FROM add_p WHERE CONCAT(p_name) LIKE '%$filtervalues%'";
					...
					if ($row=mysqli_num_rows($query_run) > 0)
				    {
						...
						foreach ($query_run as $items) 
						{
							...
							<tr>
								...
								<td><?= $items['pb_price'];  ?></td>
								<td><?= $items['ph_price'];  ?></td>
								<td><?= $items['ps_price'];  ?></td>
								<td>
								<a href='update.php?id=<?php echo $items['id']; ?>' class='btn btn-warning'>Update </a>
								</td>
								<td>
								<a href='delate.php?id=<?php echo $items['id']; ?>' class='btn btn-Danger'>Delate </a>
							</td>
